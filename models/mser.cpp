...
MSER::Region::Region(int level, int pixel, int x, int y) : level_(level), pixel_(pixel), area_(0),
  variation_(std::numeric_limits<double>::infinity()),
  stable_(false), parent_(0), child_(0), next_(0)
{
    boundRect_ = cv::Rect(x, y, 1, 1);
...
{
    ++area_;
    int x1 = std::min(boundRect_.x, x);
    int x2 = std::max(boundRect_.br().x - 1, x);
    int y1 = std::min(boundRect_.y, y);
    int y2 = std::max(boundRect_.br().y - 1, y);

    boundRect_.x = x1;
    boundRect_.y = y1;
    boundRect_.width = x2 - x1 + 1;
    boundRect_.height = y2 - y1 + 1;
...
void MSER::Region::merge(Region * child)
{
    ...
    area_ += child->area_;
    int x1 = std::min(boundRect_.x, child->boundRect_.x);
    int x2 = std::max(boundRect_.br().x - 1, child->boundRect_.br().x-1);

    int y1 = std::min(boundRect_.y, child->boundRect_.y);
    int y2 = std::max(boundRect_.br().y - 1, child->boundRect_.br().y-1);
...
void MSER::Region::save(double minDiversity, std::vector<Region> & regions)
{
    if (stable_) {
...
maxVariation_(maxVariation), minDiversity_(minDiversity)
{
    // Parameter check
    assert(delta > 0);
    assert(minArea >= 0.0);
...
void MSER::getRegions(cv::Mat input, std::vector<Region> &regions)
{
    ...
    const uchar *bits= input.data;
    regionStack.push_back(new Region);
...
// step_3:
    regionStack.push_back(new Region(curLevel, curPixel));
    ...
    for (;;) {
        ...
        if(hasLowerPoint){
            regionStack.push_back(new Region(curLevel, curPixel, x, y));
            hasLowerPoint = false;
            continue;
        }
        ...
        if (newPixelGreyLevel != curLevel) {
            ...
            // Then go to 4.
            processStack(newPixelGreyLevel, curPixel, curPixel % width, curPixel / width, regionStack);
        }
    }
}

void MSER::processStack(int newPixelGreyLevel, int pixel, int x, int y,  std::vector<Region *> & regionStack)
{
    ...
    do {
        ...
        if (newPixelGreyLevel < regionStack.back()->level_) {
            regionStack.push_back(new Region(newPixelGreyLevel, pixel, x, y ));
            // if (poolIndex_ == pool_.size())
